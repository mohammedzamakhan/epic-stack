...
export class AuthApi {
   ...
   */
  async onboarding(onboardingData: OnboardingRequest): Promise<OnboardingApiResponse> {
    try {
      // Use form data for onboarding to match web app expectations
      const response = await this.httpClient.postForm<OnboardingApiResponse['data']>(
   ...
   */
  async refreshTokens(refreshToken: string, userId: string): Promise<RefreshApiResponse> {
    try {
  ...
  async refreshSession(): Promise<RefreshApiResponse> {
    ...
    return {
      ...
      message: 'refreshSession requires refreshToken and userId parameters. Use refreshTokens instead.',
      status: 400,
    }
  }
 ...
 */
export const OAUTH_PROVIDER_CONFIGS: Record<SupportedOAuthProvider, OAuthProviderConfig> = {
  github: {
    name: 'github',
    displayName: 'GitHub',
    iconName: 'github',
