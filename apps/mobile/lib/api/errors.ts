 ...
 */
export function parseValidationErrors(errors: Array<{ field: string; message: string }>): AppError[] {
...
export function createAuthError(
  type: 'validation' | 'authentication' | 'network' | 'server' | 'banned' | 'rate_limit' | 'bot_detection',
  message: string,
  field?: string,
  details?: Record<string, any>
): AuthError {
...
export class ErrorHandler {
  ...
  async retryAll(): Promise<void> {
    const retryPromises = Array.from(this.retryCallbacks.values()).map(callback => callback())
