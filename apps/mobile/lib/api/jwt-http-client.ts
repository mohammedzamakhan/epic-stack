...
export interface JWTHttpClientConfig {
  ...
  onTokenRefresh?: (accessToken: string, refreshToken: string, userId: string) => Promise<{
    accessToken: string
    refreshToken: string
    expiresIn: number
    expiresAt: string
...
export class JWTHttpClient extends HttpClient {
  ...
  async request<T>(
    ...
    if (!isAuthEndpoint && !response.success && response.status === 401 && this.onTokenRefresh) {
      try {
  ...
  private isAuthenticationEndpoint(url: string): boolean {
    ...
    return authEndpoints.some(endpoint => url.endsWith(endpoint)) ||
           url.includes('/api/auth/') && (url.includes('/callback') || url.includes('/social'))
  }
