  ...
  const authenticate = useCallback(async (provider: string) => {
    ...
    try {
      ...
      if (result.success && result.code) {
        ...
        await socialLogin(provider, result.code, result.state, options.redirectTo)
        options.onSuccess?.(result)
      } else {
        const errorMessage = result.errorDescription || `OAuth authentication failed for ${provider}`
        setError(errorMessage)
        options.onError?.(errorMessage)
      }
    } catch (err) {
...
export function useOAuthCallback() {
  ...
  const handleCallback = useCallback(async (url: string, provider: string) => {
