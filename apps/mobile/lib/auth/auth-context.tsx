...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const login = useCallback(async (credentials: LoginCredentials) => {
    try {
      ...
      console.log('📡 Auth Context: API response:', { success: response.success, error: response.error, message: response.message })
      ...
      if (response.success && response.data?.user && response.data?.accessToken) {
        ...
        console.log('❌ Auth Context: Login failed:', response.error || response.message)
  ...
  const signup = useCallback(async (email: string) => {
    try {
      ...
      if (response.success) {
        ...
        dispatch({
          type: 'AUTH_ERROR',
          payload: { error: response.error || response.message || 'Signup failed' },
        })
      }
    } catch (error) {
      dispatch({
  ...
  const socialLogin = useCallback(async (
    ...
    try {
      ...
      if (code) {
        ...
        const response = await jwtAuthApi.socialCallback(provider, code, state)
        ...
        if (response.success && response.data?.user && response.data?.accessToken) {
          ...
          dispatch({
            type: 'AUTH_ERROR',
            payload: { error: response.error || response.message || 'OAuth authentication failed' },
          })
        }
      } else {
      ...
      dispatch({
        type: 'AUTH_ERROR',
        payload: { error: error instanceof Error ? error.message : 'Social login failed' },
      })
    }
  }, [])
  ...
  const refreshTokens = useCallback(async () => {
    try {
      ...
      const response = await jwtAuthApi.refreshTokens(currentTokens.refreshToken, user.id)
  ...
  // Verify function
  const verify = useCallback(async (data: { code: string; type: string; target: string; redirectTo?: string }) => {
    try {
      dispatch({ type: 'AUTH_START' })
      ...
      if (response.success) {
        ...
        dispatch({
          type: 'AUTH_ERROR',
          payload: { error: response.error || response.message || 'Verification failed' },
        })
      }
    } catch (error) {
      dispatch({
  ...
  const onboarding = useCallback(async (data: {
    ...
    try {
      ...
      if (response.success && response.data?.user && response.data?.accessToken) {
        ...
        dispatch({
          type: 'AUTH_ERROR',
          payload: { error: response.error || response.message || 'Account creation failed' },
        })
      }
    } catch (error) {
      dispatch({
