...
const ERROR_MESSAGES: Record<ErrorCategory, Record<string, string>> = {
  ...
  [ErrorCategory.OAUTH]: {
    ...
    provider_error: 'There was an error with the login provider. Please try again.',
  ...
  [ErrorCategory.BOT_DETECTION]: {
    default: 'Access denied. If you believe this is an error, please contact support.',
...
export function getErrorMessage(error: AppErrorType): string {
  ...
  return categoryMessages.default || ERROR_MESSAGES[ErrorCategory.UNKNOWN].default
}

/**
 * Gets a specific error message based on error properties
 */
function getSpecificMessage(error: AppErrorType, messages: Record<string, string>): string | null {
  switch (error.category) {
    case ErrorCategory.NETWORK:
      ...
      if (error.message.includes('ECONNREFUSED') && messages.connection_refused) {
    ...
    case ErrorCategory.AUTHENTICATION:
      ...
      if (error.statusCode === 401 && messages.unauthorized) return messages.unauthorized
      break

    case ErrorCategory.VALIDATION:
      if (error.message.includes('email') && messages.invalid_email) return messages.invalid_email
      if (error.message.includes('password') && messages.invalid_password) return messages.invalid_password
      if (error.message.includes('required') && messages.required_field) return messages.required_field
      if (error.message.includes('exists') && messages.email_exists) return messages.email_exists
      break

    case ErrorCategory.OAUTH:
      const oauthError = error as OAuthError
      if (oauthError.cancelled && messages.cancelled) return messages.cancelled
      if (error.message.includes('provider') && messages.provider_error) return messages.provider_error
      if (error.message.includes('authorization') && messages.authorization_failed) {
        return messages.authorization_failed
      }
      break

    case ErrorCategory.RATE_LIMIT:
      if (error.message.includes('quota') && messages.quota_exceeded) return messages.quota_exceeded
      break

    case ErrorCategory.BOT_DETECTION:
      if (error.message.includes('suspicious') && messages.suspicious_activity) {
    ...
    case ErrorCategory.SERVER:
      ...
      if (serverError.maintenance && messages.maintenance) return messages.maintenance
      if (error.statusCode === 404 && messages.not_found) return messages.not_found
      if (error.statusCode === 500 && messages.internal_error) return messages.internal_error
      break
  }
