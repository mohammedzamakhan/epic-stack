...
export class RetryManager {
  ...
  async execute<T>(fn: () => Promise<T>): Promise<T> {
    ...
    while (this.state.attempt < this.config.maxAttempts) {
      try {
        ...
        if (this.state.attempt >= this.config.maxAttempts || !this.shouldRetry(this.state.lastError)) {
  ...
  private shouldRetry(error: AppErrorType): boolean {
    ...
    if (this.config.retryableErrors && !this.config.retryableErrors.includes(error.category)) {
      return false
    }
