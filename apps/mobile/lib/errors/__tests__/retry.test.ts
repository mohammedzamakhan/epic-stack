...
describe('RetryManager', () => {
  ...
  it('calculates exponential backoff delay', async () => {
    const retryManager = new RetryManager({ maxAttempts: 3, baseDelay: 1000, backoffFactor: 2 })
    const mockFn = jest.fn()
      .mockRejectedValueOnce(new Error('Network connection failed'))
      .mockRejectedValueOnce(new Error('Network connection failed'))
      .mockResolvedValue('success')
  ...
  it('respects max delay', async () => {
    ...
    calls.forEach(call => {
      if (call[1] > 0) { // Ignore immediate calls
...
describe('retry utility functions', () => {
  ...
  it('shouldShowRetryButton returns true for retryable errors', () => {
    const retryableError = categorizeError({ message: 'Network connection failed' })
    const nonRetryableError = categorizeError({ status: 400, message: 'Bad request' })
    const rateLimitError = categorizeError({ status: 429, message: 'Rate limit exceeded' })
