...
export function InputOTP({ maxLength, value, onChange, disabled, style, children }: InputOTPProps) {
...
export function InputOTPSlot({ index }: InputOTPSlotProps) {
  ...
  const { value, onChange, maxLength, disabled, focusedIndex, setFocusedIndex } = context
  ...
  const handleChangeText = (text: string) => {
    ...
    if (text.length > 1) {
      console.log(`InputOTP Slot ${index}: Handling paste with ${text.length} characters`)
