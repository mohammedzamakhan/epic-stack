...
| CC7.2 Security Monitoring | Audit logs, PostHog/Cloudflare monitoring  | `packages/audit`, `packages/observability` |
...
| Control                | Implementation          | Evidence                          |
| ---------------------- | ----------------------- | --------------------------------- |
| A1.1 Capacity Planning | Cloudflare auto-scaling | Workers configuration             |
...
| C1.2 Data Disposal       | Cascade deletes, retention policies | Drizzle schema, audit archival          |
...
<Warning>
	Epic Stack is not HIPAA-certified out of the box. Additional measures are
...
```typescript
await db.insert(AuditLogRetentionPolicy).values({
	organizationId,
	retentionDays: 365,
	hotStorageDays: 180,
	archiveEnabled: true,
