...
export async function loader({ request }: LoaderFunctionArgs) {
	...
	const [note] = await db
		...
		.where(
			and(
				eq(Note.id, noteId),
				eq(Note.organizationId, userOrganization.id), // Prevents IDOR
			),
		)
		.limit(1)
}
...
```typescript
// ✅ Drizzle parameterizes query values
const [user] = await db
  .select()
  .from(User)
  .where(eq(User.email, userInput)) // Safe from SQL injection
  .limit(1)
...
<Note>
	This approach works because Epic Stack avoids GET-based mutations. All
...
await db
	...
	.where(eq(OrganizationInvitation.id, invitationId)) // Any user can delete any invitation!
...
await db
	...
	.where(
		and(
			...
			eq(OrganizationInvitation.organizationId, userOrganization.id),
		),
	)
```
...
Endpoints that need a tighter or different window than the tiers above use a
dedicated DB-backed sliding-window limiter (`apps/app/app/utils/rate-limit.server.ts`),
e.g. forgot-password (3/hour) and the translate API (30/min).

Additional protections:

- Honeypot fields on forms
- IP blacklist and request tracking (`@repo/common/ip-tracking`)
