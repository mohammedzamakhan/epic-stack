...
const [note] = await db
	.select()
	.from(Note)
	.where(and(eq(Note.id, noteId), eq(Note.organizationId, userOrganizationId)))
	.limit(1)

// ❌ Bad - No ownership check (IDOR vulnerability)
const [note] = await db.select().from(Note).where(eq(Note.id, noteId)).limit(1)
```
...
await db.all(sql`SELECT * FROM users WHERE id = ${userId}`)

// ❌ Bad - String concatenation (SQL injection risk)
await db.all(sql.raw(`SELECT * FROM users WHERE id = '${userId}'`))
```
...
const [user] = await db.select().from(User).where(eq(User.id, userId)).limit(1) // Exposes sensitive data
```
