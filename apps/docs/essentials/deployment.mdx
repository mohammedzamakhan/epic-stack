...
The **CMS** deploys to **Cloudflare Workers** (via OpenNext), backed by
Cloudflare **D1** (SQLite) and **R2** (media storage) — not Fly.io, and not
MongoDB. See [CMS storage](/docs/cms-storage.md). The marketing **Website**
and **Sites** deploy to Cloudflare Pages.
