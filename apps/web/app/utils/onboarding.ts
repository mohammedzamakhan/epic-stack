...
export const DEFAULT_ONBOARDING_STEPS = [
  ...
  },
  {
    ...
    description: 'Experience AI-powered assistance for your notes (auto-completes when you use AI)',
    actionConfig: {
      type: 'navigate' as const,
      target: '/notes',
      label: 'Go to Notes'
  ...
  for (const step of DEFAULT_ONBOARDING_STEPS) {
    await prisma.onboardingStep.upsert({
      ...
      update: {
        ...
        detectConfig: step.detectConfig ? JSON.stringify(step.detectConfig) : null,
...
export async function getOnboardingProgress(
  ...
  const stepsWithProgress: OnboardingStepWithProgress[] = steps.map(step => {
    ...
    return {
      ...
      actionConfig: step.actionConfig ? JSON.parse(step.actionConfig) : undefined,
      detectConfig: step.detectConfig ? JSON.parse(step.detectConfig) : undefined,
      sortOrder: step.sortOrder,
  ...
  const completedCount = stepsWithProgress.filter(step => step.isCompleted).length
...
export async function autoDetectCompletedSteps(
  ...
  for (const step of stepsWithDetection) {
    ...
    try {
      const detectConfig = JSON.parse(step.detectConfig) as OnboardingStepDetectConfig
      const isCompleted = evaluateDetectionCondition(detectConfig.condition, detectionData)
...
async function getDetectionData(userId: string, organizationId: string) {
  const [notesCount, membersCount, organization, integrationsCount, invitationsCount] = await Promise.all([
  ...
  return {
    ...
    hasCompletedProfile: !!(organization?.name && organization?.name.trim() !== '' && organization?.slug && organization?.slug.trim() !== ''),
...
function evaluateDetectionCondition(condition: string, data: Record<string, any>): boolean {
  switch (condition) {
    case 'hasNotes':
