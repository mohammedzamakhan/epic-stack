  ...
  try {
    ...
    if (isOAuth1) {
      ...
      if (!provider || !('getRequestTokenContext' in provider)) {
        throw new Error('Trello provider not found or does not support OAuth 1.0a')
      }
      
      // Get the stored context using the oauth_token (request token)
      const tokenContext = await (provider as any).getRequestTokenContext(oauthToken!)
      
      if (!tokenContext) {
        throw new Error('OAuth request token not found or expired. Please restart the authorization process.')
      }
