...
import { verifyUserPassword, getPasswordHash, checkIsCommonPassword } from '#app/utils/auth.server.ts'
...
export async function changePasswordAction({ userId, formData }: SecurityActionArgs) {
...
export async function setPasswordAction({ userId, formData }: SecurityActionArgs) {
  const submission = await parseWithZod(formData, {
    async: true,
...
export async function enable2FAAction({ formData, userId }: SecurityActionArgs) {
  const submission = await parseWithZod(formData, {
