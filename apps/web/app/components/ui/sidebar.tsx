...
function SidebarProvider({
	...
	React.useEffect(() => {
		if (sidebarFetcher.state === 'idle' && isOptimistic.current) {
			// Reset the optimistic update flag when the fetcher is done
			isOptimistic.current = false

			// If there was an error, revert to the prop value
			if (sidebarFetcher.data?.error) {
				setOptimisticOpen(openProp)
			}
		}
	}, [sidebarFetcher.state, sidebarFetcher.data, openProp])
	...
	const setOpen = React.useCallback(
		(value: boolean | ((value: boolean) => boolean)) => {
			...
			void sidebarFetcher.submit(
				{ isCollapsed: String(!newOpenState) },
				{
					method: 'POST',
		...
		[setOpenProp, optimisticOpen, sidebarFetcher],
	)
