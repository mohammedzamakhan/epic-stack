      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Connect this note to external services to automatically share updates.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
                ...
                {connections.map((connection) => (
                  <ConnectedChannelItem
                    ...
                    isDisconnecting={
                      ...
                      disconnectFetcher.formData?.get('connectionId') === connection.id
                    }
                  />
                ))}
              </div>
            ...
            {!hasAvailableIntegrations && !hasConnections && (
              <div className="text-center py-4">
                <Icon name="link-2" className="mx-auto h-8 w-8 text-muted-foreground/50" />
                <p className="mt-2 text-sm text-muted-foreground">
                  No integrations available. Connect your organization to external services first.
                </p>
              </div>
            )}
          </div>
...
function ConnectedChannelItem({ connection, onDisconnect, isDisconnecting }: ConnectedChannelItemProps) {
    ...
    <div className="flex items-center justify-between p-3 border rounded-lg">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground capitalize">{providerName}</p>
        </div>
      </div>
      <StatusButton
        variant="outline"
...
function AddConnectionForm({ integrations, onConnect, isConnecting }: AddConnectionFormProps) {
      ...
      {selectedIntegration && (
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Note: The bot may need to be invited to private channels before it can post messages.
          </p>
          <Select
                ...
                availableChannels.map((channel) => (
                  <SelectItem key={channel.id} value={channel.id}>
                    <div className="flex items-center space-x-2">
                      ...
                      <Badge
                        variant={channel.type === 'private' ? 'secondary' : 'outline'}
                        className="text-xs"
                      >
                        {channel.type}
                      </Badge>
