...
export class SSORetryManager {
	 ...
	 */
	async retryOIDCDiscovery(
		discoveryFunction: () => Promise<any>,
		issuerUrl: string,
	 ...
	 */
	async retryUserInfoFetch(
		userInfoFunction: () => Promise<any>,
		provider: string,
	): Promise<RetryResult<any>> {
		return this.executeWithRetry(
			userInfoFunction,
	...
	withTimeout<T>(
		...
		return async (): Promise<T> => {
			...
			try {
				// Note: This assumes the operation supports AbortSignal
				// For operations that don't, we'll use Promise.race
