...
const inputGroupAddonVariants = cva(
	"text-muted-foreground flex h-auto cursor-text items-center justify-center gap-2 py-1.5 text-sm font-medium select-none group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-[calc(var(--radius)-5px)] [&>svg:not([class*='size-'])]:size-4",
	{
		variants: {
			align: {
				'inline-start':
					'order-first ltr:pl-2 rtl:pr-2 [&:has(button)]:ltr:ml-[-0.3rem] [&:has(button)]:rtl:mr-[-0.3rem] [&:has(kbd)]:ltr:ml-[-0.15rem] [&:has(kbd)]:rtl:mr-[-0.15rem]',
				'inline-end':
					'order-last ltr:pr-2 rtl:pl-2 [&:has(button)]:ltr:mr-[-0.3rem] [&:has(button)]:rtl:ml-[-0.3rem] [&:has(kbd)]:ltr:mr-[-0.15rem] [&:has(kbd)]:rtl:ml-[-0.15rem]',
				'block-start':
