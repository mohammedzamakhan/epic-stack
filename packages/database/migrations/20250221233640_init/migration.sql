...
--------------------------------- Manual Seeding --------------------------
-- Roles and permissions are seeded here. To customise them, dump SQL from
-- the control-plane database (`sqlite3 packages/database/db/data.db .dump`)
-- and replace the INSERT statements below. Development seed data also lives
-- in packages/database/seed.ts (`npm run db:seed`).
