	...
	describe('OAuth Operations', () => {
		...
		describe('handleOAuthCallback', () => {
			it('should handle OAuth callback successfully', async () => {
				...
				vi.mocked(integrationManager.handleOAuthCallback).mockResolvedValue(
					expectedIntegration,
				)
				...
				expect(integrationManager.handleOAuthCallback).toHaveBeenCalledWith(
					providerName,
					params,
				)
			})
	...
	describe('Channel Operations', () => {
		describe('getAvailableChannels', () => {
			it('should get available channels', async () => {
				...
				vi.mocked(integrationManager.getAvailableChannels).mockResolvedValue(
					expectedChannels,
				)
				...
				expect(integrationManager.getAvailableChannels).toHaveBeenCalledWith(
					integrationId,
				)
			})
		})
	...
	describe('Connection Management', () => {
		describe('connectNoteToChannel', () => {
			it('should connect note to channel successfully', async () => {
				...
				vi.mocked(integrationManager.connectNoteToChannel).mockResolvedValue(
					expectedConnection,
				)

				const result = await service.connectNoteToChannel(
					noteId,
					integrationId,
					channelId,
					config,
		...
		describe('disconnectNoteFromChannel', () => {
			it('should disconnect note from channel', async () => {
				...
				vi.mocked(
					integrationManager.disconnectNoteFromChannel,
				).mockResolvedValue()
		...
		describe('getNoteConnections', () => {
			it('should get note connections', async () => {
				...
				expect(integrationManager.getNoteConnections).toHaveBeenCalledWith(
					noteId,
				)
			})
		})
	...
	describe('Note Update Handling', () => {
		describe('handleNoteUpdate', () => {
			it('should handle note update', async () => {
				...
				expect(integrationManager.handleNoteUpdate).toHaveBeenCalledWith(
					noteId,
					changeType,
					userId,
				)
	...
	describe('Token Management', () => {
		describe('refreshTokens', () => {
			it('should refresh integration tokens', async () => {
				...
				vi.mocked(
					integrationManager.refreshIntegrationTokens,
				).mockResolvedValue(expectedIntegration)
				...
				expect(
					integrationManager.refreshIntegrationTokens,
				).toHaveBeenCalledWith(integrationId)
			})
		})
		...
		describe('shouldRefreshToken', () => {
			it('should check if token needs refresh', async () => {
				...
				expect(TokenRefreshManager.shouldRefreshToken).toHaveBeenCalledWith(
					tokenData,
				)
			})
		})
		...
		describe('isTokenExpired', () => {
			it('should check if token is expired', async () => {
				...
				expect(result).toBe(true)
				expect(TokenRefreshManager.isTokenExpired).toHaveBeenCalledWith(
					tokenData,
				)
			})
		})
	...
	describe('Integration Management', () => {
		describe('validateIntegrationConnections', () => {
			it('should validate integration connections', async () => {
				...
				const result =
					await service.validateIntegrationConnections(integrationId)
		...
		describe('getIntegrationStatus', () => {
			it('should get integration status', async () => {
				...
				vi.mocked(integrationManager.getIntegrationStatus).mockResolvedValue(
					expectedStatus,
				)
				...
				expect(integrationManager.getIntegrationStatus).toHaveBeenCalledWith(
					integrationId,
				)
			})
		})
		...
		describe('disconnectIntegration', () => {
			it('should disconnect integration', async () => {
				...
				expect(integrationManager.disconnectIntegration).toHaveBeenCalledWith(
					integrationId,
				)
			})
		})
		...
		describe('getOrganizationIntegrations', () => {
			it('should get organization integrations without type filter', async () => {
				...
				vi.mocked(
					integrationManager.getOrganizationIntegrations,
				).mockResolvedValue(expectedIntegrations)
			...
			it('should get organization integrations with type filter', async () => {
				...
				const result = await service.getOrganizationIntegrations(
					organizationId,
					type,
				)
				...
				expect(
					integrationManager.getOrganizationIntegrations,
				).toHaveBeenCalledWith(organizationId, type)
			})
		})
	})
	...
	describe('Provider Management', () => {
		describe('registerProvider', () => {
			it('should register a provider', () => {
				...
				expect(integrationManager.registerProvider).toHaveBeenCalledWith(
					mockProvider,
				)
			})
		})
		...
		describe('getProvider', () => {
			it('should get a provider by name', () => {
				...
				expect(integrationManager.getProvider).toHaveBeenCalledWith(
					providerName,
				)
			})
		})
	...
	describe('Additional Integration Methods', () => {
		describe('getIntegration', () => {
			it('should get integration by ID', async () => {
				...
				expect(integrationManager.getIntegration).toHaveBeenCalledWith(
					integrationId,
				)
			})
		})
		...
		describe('getIntegrationConnections', () => {
			it('should get integration connections', async () => {
				...
				vi.mocked(
					integrationManager.getIntegrationConnections,
		...
		describe('getIntegrationStats', () => {
			it('should get integration statistics', async () => {
				...
				vi.mocked(integrationManager.getIntegrationStats).mockResolvedValue(
					mockStats,
				)
				...
				expect(integrationManager.getIntegrationStats).toHaveBeenCalledWith(
					integrationId,
				)
			})
		})
...
describe('Singleton instance', () => {
	...
	it('should be the same instance across imports', async () => {
		const { integrationService: importedService } = await import(
			'../../src/service'
		)
