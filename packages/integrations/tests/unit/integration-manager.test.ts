...
import { MockProvider, createTestIntegration, createTestChannels } from '../utils/test-helpers'
...
describe('IntegrationManager', () => {
  ...
  describe('OAuth Flow Management', () => {
    ...
    it('should initiate OAuth flow', async () => {
      mockProvider.getAuthUrl.mockResolvedValue('https://example.com/auth?state=test-state')

      const result = await manager.initiateOAuth('org-123', 'mock', 'http://callback.com')
  ...
  describe('Integration CRUD Operations', () => {
    ...
    it('should get organization integrations with type filter', async () => {
      ...
      const result = await manager.getOrganizationIntegrations('org-123', 'communication')
    ...
    it('should update integration config', async () => {
      ...
      const result = await manager.updateIntegrationConfig('integration-123', { setting: 'value' })
      expect(result).toEqual(mockIntegration)
    })
    ...
    it('should disconnect integration', async () => {
      const mockIntegration = createTestIntegration()
      vi.mocked(prisma.integration.findUnique).mockResolvedValue(mockIntegration)
      vi.mocked(prisma.noteIntegrationConnection.deleteMany).mockResolvedValue({ count: 2 })
      vi.mocked(prisma.integrationLog.deleteMany).mockResolvedValue({ count: 5 })
  ...
  describe('Connection Management', () => {
    ...
    it('should connect note to channel', async () => {
      ...
      mockProvider.getAvailableChannels.mockResolvedValue(mockChannelsWithTarget)
    ...
    it('should disconnect note from channel', async () => {
      ...
      vi.mocked(prisma.noteIntegrationConnection.findUnique).mockResolvedValue(mockConnection as any)
      vi.mocked(prisma.noteIntegrationConnection.delete).mockResolvedValue(mockConnection as any)
      vi.mocked(prisma.integrationLog.create).mockResolvedValue({} as any)
  ...
  describe('Channel Operations', () => {
    ...
    it('should handle channel fetch error', async () => {
      ...
      await expect(manager.getAvailableChannels('integration-123')).rejects.toThrow('API Error')
    })
  })
  ...
  describe('Message Posting', () => {
    ...
    it('should handle note update with connections', async () => {
      ...
      vi.mocked(prisma.noteIntegrationConnection.update).mockResolvedValue({} as any)
      vi.mocked(prisma.integrationLog.create).mockResolvedValue({} as any)
  ...
  describe('Validation and Health Checks', () => {
    ...
    it('should validate integration connections', async () => {
      ...
      const result = await manager.validateIntegrationConnections('integration-123')
