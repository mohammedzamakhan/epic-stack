...
describe('BaseIntegrationProvider', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('INTEGRATIONS_OAUTH_STATE_SECRET', 'test-secret-key-for-oauth-state-validation-12345')
  })
  ...
  describe('handleApiError', () => {
    ...
    it('should include JSON error message when available', async () => {
      const errorData = { error: 'invalid_request', message: 'Invalid parameters' }
      const mockResponse = {
        status: 400,
    ...
    it('should include raw text when JSON parsing fails', async () => {
      ...
      await expect(
        provider['handleApiError'](mockResponse, 'Test context')
      ).rejects.toThrow('Test context: 500 Internal Server Error - Raw error text')
    })
...
describe('Global provider registry', () => {
  ...
  it('should persist across different imports', async () => {
    ...
    const { providerRegistry: importedRegistry } = await import('../../src/provider')
