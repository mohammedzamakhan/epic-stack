...
describe('NoteEventHandler - Extended Coverage', () => {
  ...
  describe('Error Handling', () => {
    it('should handle integration manager errors gracefully', async () => {
      ...
      mockIntegrationManager.handleNoteUpdate.mockRejectedValue(new Error('Integration error'))
    ...
    it('should handle missing note data', async () => {
      ...
      const result = await handler.handleNoteCreated('nonexistent-note', 'user-123')
    ...
    it('should handle database errors', async () => {
      mockPrisma.organizationNote.findUnique.mockRejectedValue(new Error('Database error'))
