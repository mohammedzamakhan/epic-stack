...
describe('GitHubProvider - Utility Methods', () => {
  ...
  describe('Repository Filtering', () => {
    it('should filter out archived repositories', () => {
      const repos = [
        { id: 1, name: 'active-repo', archived: false, disabled: false, permissions: { push: true } },
        { id: 2, name: 'archived-repo', archived: true, disabled: false, permissions: { push: true } },
      ]
      
      const filtered = repos.filter(repo => !repo.archived && !repo.disabled && repo.permissions?.push)
    ...
    it('should filter out disabled repositories', () => {
      const repos = [
        { id: 1, name: 'active-repo', archived: false, disabled: false, permissions: { push: true } },
        { id: 2, name: 'disabled-repo', archived: false, disabled: true, permissions: { push: true } },
      ]
    ...
    it('should filter out repositories without push permissions', () => {
      const repos = [
        { id: 1, name: 'push-repo', archived: false, disabled: false, permissions: { push: true } },
        { id: 2, name: 'no-push-repo', archived: false, disabled: false, permissions: { push: false } },
      ]
