...
describe('SlackProvider - Message Posting', () => {
	...
	describe('postMessage', () => {
		...
		it('should skip posting for mock tokens', async () => {
			...
			await expect(
				provider.postMessage(connectionWithMockToken, mockMessageData),
		...
		it('should post message with blocks format by default', async () => {
			...
			expect(requestBody.text).toBe(
				'✨ Test Note Title was created by Test Author',
			)

			// Check blocks structure
			expect(requestBody.blocks[0].type).toBe('section')
			expect(requestBody.blocks[0].text.text).toContain(
				'✨ *Test Note Title* was created by *Test Author*',
			)
			...
			expect(requestBody.blocks[2].elements[0].url).toBe(
		...
		it('should post message with text format when configured', async () => {
			...
			expect(requestBody.text).toContain(
				'<https://example.com/notes/123|View Note>',
			)
		})
		...
		it('should handle different change types with correct emojis', async () => {
			...
			for (const testCase of testCases) {
				...
				expect(requestBody.text).toContain(
					`${testCase.emoji} Test Note Title was ${testCase.changeType}`,
				)
			}
		})
		...
		it('should handle invalid URLs by using context instead of buttons', async () => {
			...
			const contextBlock = requestBody.blocks.find(
		...
		it('should handle missing access token', async () => {
			...
			await expect(
				provider.postMessage(connectionWithoutToken, mockMessageData),
			).rejects.toThrow('No access token available for Slack integration')
		})
		...
		it('should handle Slack API errors with specific error messages', async () => {
			const errorTestCases = [
				{
					...
					expectedMessage: 'Slack channel not found',
				},
				{
					error: 'not_in_channel',
					expectedMessage: 'Bot is not a member of this Slack channel',
				},
				{
					error: 'channel_is_archived',
					expectedMessage: 'Cannot post to archived Slack channel',
				},
				{
					error: 'msg_too_long',
					expectedMessage: 'Message is too long for Slack',
				},
				{
					error: 'rate_limited',
					expectedMessage: 'Slack API rate limit exceeded',
				},
				{
					error: 'invalid_auth',
					expectedMessage: 'Slack authentication failed',
				},
				{
					error: 'invalid_blocks',
					expectedMessage: 'Invalid Slack message format',
				},
			]
			...
			for (const testCase of errorTestCases) {
				...
				await expect(
					provider.postMessage(mockConnection, mockMessageData),
