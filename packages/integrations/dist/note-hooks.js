...
export class NoteHooks {
     ...
     */
    async afterNoteCreated(noteId, userId) {
        try {
     ...
     */
    async afterNoteUpdated(noteId, userId, previousData) {
        try {
            // Trigger integration notifications asynchronously
            setImmediate(async () => {
                try {
     ...
     */
    async beforeNoteDeleted(noteId, userId) {
        try {
            ...
            setImmediate(async () => {
                try {
                    const result = await noteEventHandler.handleNoteDeleted(noteId, userId, { title: note.title, organizationId: note.organizationId });
                    if (!result.success) {
     ...
     * @param userId - ID of the user who created the note
     */
    async onNoteCreated(noteData, userId) {
     ...
     */
    async onNoteUpdated(noteId, userId, beforeSnapshot, _afterSnapshot) {
        let previousData;
        if (beforeSnapshot) {
            previousData = {
     ...
     */
    async onNoteDeleted(noteData, userId) {
     ...
     */
    async captureNoteSnapshot(noteId) {
        try {
            const note = await prisma.organizationNote.findUnique({
                where: { id: noteId },
                select: {
                    ...
                    try {
                        switch (change.type) {
                            case 'created':
                                ...
                                break;
                            case 'updated':
                                await this.onNoteUpdated(change.noteId, change.userId, change.beforeSnapshot, change.afterSnapshot);
                                break;
                            case 'deleted':
 ...
 */
export async function triggerNoteCreated(noteId, userId) {
    await noteHooks.afterNoteCreated(noteId, userId);
}
/**
 * Call this after updating a note
 */
export async function triggerNoteUpdated(noteId, userId, previousData) {
    await noteHooks.afterNoteUpdated(noteId, userId, previousData);
}
/**
 * Call this before deleting a note
 */
export async function triggerNoteDeleted(noteId, userId) {
...
export class NoteOperationWrapper {
     ...
     */
    static async create(operation, noteId, userId) {
     ...
     */
    static async update(operation, noteId, userId, captureSnapshot = true) {
     ...
     */
    static async delete(operation, noteId, userId) {
