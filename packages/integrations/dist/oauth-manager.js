...
export class OAuthStateManager {
    static STATE_EXPIRY_MINUTES = 30;
    static getStateSecret() {
     ...
     */
    static generateState(organizationId, providerName, redirectUrl, additionalData) {
        const stateData = {
            organizationId,
            providerName,
            redirectUrl,
     ...
     */
    static createStateSignature(payload) {
...
export class OAuthCallbackHandler {
     ...
     */
    static async handleCallback(providerName, params) {
     ...
     */
    static async generateAuthUrl(organizationId, providerName, redirectUri, additionalParams) {
        const provider = providerRegistry.get(providerName);
...
export class TokenRefreshManager {
    ...
    static RETRY_DELAYS = [1000, 2000, 4000]; // Exponential backoff in ms
    static TOKEN_BUFFER_MINUTES = 5; // Refresh tokens 5 minutes before expiry
    /**
     ...
     */
    static async refreshTokenWithRetry(providerName, refreshToken, attempt = 1) {
        const provider = providerRegistry.get(providerName);
     ...
     */
    static shouldRefreshToken(tokenData) {
...
export class OAuthFlowManager {
     ...
     */
    static async startOAuthFlow(organizationId, providerName, redirectUri, additionalParams) {
        // Generate state
        const state = OAuthStateManager.generateState(organizationId, providerName, additionalParams?.redirectUrl, additionalParams);
        // Get authorization URL from provider
        const authUrl = await OAuthCallbackHandler.generateAuthUrl(organizationId, providerName, redirectUri, { ...additionalParams, state });
        return {
            authUrl,
            state,
        };
     ...
     */
    static async completeOAuthFlow(providerName, params) {
     ...
     */
    static async ensureValidToken(providerName, currentTokenData) {
        ...
        return TokenRefreshManager.refreshTokenWithRetry(providerName, currentTokenData.refreshToken);
    }
}
