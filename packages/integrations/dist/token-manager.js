...
import { integrationEncryption } from './encryption';
/**
...
export class TokenManager {
     ...
     */
    async storeTokenData(integrationId, tokenData) {
        try {
     ...
     */
    async getTokenData(integrationId) {
        try {
     ...
     */
    async getValidAccessToken(integration, provider) {
        try {
     ...
     */
    async refreshToken(integration, provider, refreshToken) {
        try {
            ...
            // Log failed refresh
            await this.logTokenOperation(integration.id, 'token_refresh', 'error', error instanceof Error ? error.message : 'Unknown error');
     ...
     */
    async validateIntegrationToken(integrationId) {
        try {
     ...
     */
    async checkTokensNeedingRefresh(organizationId) {
        try {
     ...
     */
    async revokeToken(integrationId, provider) {
        try {
            ...
            await this.logTokenOperation(integrationId, 'token_revoke', 'error', error instanceof Error ? error.message : 'Unknown error');
            return false;
        }
    }
    /**
     ...
     */
    async logTokenOperation(integrationId, action, status, errorMessage) {
        try {
    ...
    isReauthError(error) {
        ...
        return (reauthPatterns.some(pattern => errorMessage.includes(pattern)) ||
