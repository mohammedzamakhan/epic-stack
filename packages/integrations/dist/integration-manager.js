...
export class IntegrationManager {
    ...
    async handleOAuthCallback(providerName, params) {
        ...
        await this.logIntegrationActivity(integration.id, 'oauth_complete', 'success', { provider: providerName });
        return integration;
    }
    // Integration CRUD Operations
    /**
    ...
    async updateIntegrationConfig(integrationId, config) {
        ...
        await this.logIntegrationActivity(integrationId, 'config_update', 'success', { config });
        return integration;
    }
    /**
     ...
     */
    async connectNoteToChannel(params) {
        ...
        await this.logIntegrationActivity(integrationId, 'connection_create', 'success', {
            noteId,
    ...
    async disconnectNoteFromChannel(connectionId) {
        ...
        // Log disconnection
        await this.logIntegrationActivity(connection.integrationId, 'connection_delete', 'success', {
    ...
    async getAvailableChannels(integrationId) {
        ...
        try {
            ...
            await this.logIntegrationActivity(integrationId, 'fetch_channels', 'success', { channelCount: channels.length });
            return channels;
        }
        catch (error) {
            // Log error
            await this.logIntegrationActivity(integrationId, 'fetch_channels', 'error', undefined, error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    ...
    async handleNoteUpdate(noteId, changeType, userId) {
        ...
        const results = await Promise.allSettled(connections.map(connection => this.postMessageToConnection(connection, message)));
        // Log overall results
     ...
     */
    async postMessageToConnection(connection, message) {
        try {
            ...
            // Log successful post
            await this.logIntegrationActivity(connection.integrationId, 'post_message', 'success', {
                noteId: connection.noteId,
                channelId: connection.externalId,
            ...
            await this.logIntegrationActivity(connection.integrationId, 'post_message', 'error', {
                noteId: connection.noteId,
                channelId: connection.externalId,
                changeType: message.changeType
     ...
     */
    async refreshIntegrationTokens(integrationId) {
        const integration = await this.getIntegration(integrationId);
        if (!integration) {
            throw new Error('Integration not found');
        }
        ...
        try {
            ...
            await this.logIntegrationActivity(integrationId, 'token_refresh', 'success');
            return updatedIntegration;
        }
        catch (error) {
            // Log error
            await this.logIntegrationActivity(integrationId, 'token_refresh', 'error', undefined, error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    ...
    async validateIntegrationConnections(integrationId) {
        ...
        await this.logIntegrationActivity(integrationId, 'validate_connections', errors.length > 0 ? 'error' : 'success', { valid, invalid, totalConnections: connections.length });
