...
export class NoteEventHandler {
     ...
     */
    async handleNoteCreated(noteId, userId) {
        try {
     ...
     * @returns Processing result
     */
    async handleNoteUpdated(noteId, userId, previousData) {
        try {
     ...
     * @returns Processing result
     */
    async handleNoteDeleted(noteId, userId, noteData) {
        try {
            const event = {
                noteId,
                changeType: 'deleted',
    ...
    async processNoteEvent(event) {
        try {
            ...
            const integrations = await integrationManager.getOrganizationIntegrations(event.organizationId);
            ...
            // Trigger integration notifications
            await integrationManager.handleNoteUpdate(event.noteId, event.changeType, event.userId);
            return {
                success: true,
     ...
     */
    async getEventStats(organizationId, timeRange = 24) {
        try {
            // Get integration logs for the organization within the time range
            const integrations = await integrationManager.getOrganizationIntegrations(organizationId);
...
//# sourceMappingURL=note-event-handler.js.map