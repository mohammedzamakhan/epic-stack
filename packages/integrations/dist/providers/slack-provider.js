...
export class SlackProvider extends BaseIntegrationProvider {
     ...
     */
    async getAuthUrl(organizationId, redirectUri, additionalParams) {
     ...
     */
    async getAvailableChannels(integration) {
        try {
     ...
     */
    async postMessage(connection, message) {
        try {
            // Get the access token (in production, this would be decrypted)
            ...
            const connectionConfig = connection.config ? JSON.parse(connection.config) : {};
     ...
     */
    async validateConnection(_connection) {
     ...
     */
    formatSlackBlocks(message, includeContent = true) {
     ...
     */
    formatSlackText(message, includeContent = true) {
        const changeTypeEmoji = this.getChangeTypeEmoji(message.changeType);
